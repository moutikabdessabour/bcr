list_of_codes <- data.frame(name = c("Addoha", "AFMA", "Afric Indus.", "Afriquia Gaz",
...
get_html <- function(url){
    s_resp <- tryCatch({
        ...
        error = function(e){
            stop("Can't fetch data from API\n Check if your computer is connected to the internet.")
        }
    )
    fromJSON(rawToChar(content(s_resp, "raw")[-(1:3)])) -> rsp
    if(is.null(rsp) || is.null(rsp$result)){
         stop("server returned Nil response")
    }
    #if(nrow(result)==1 & length(result[[1,1]])>0)
        #setNames(do.call(data.frame, result), nm=names(result)) else
    as.data.frame(rsp$result)
}
...
check_internet <- function(){
    tryCatch({
            GET("http://leboursier.ma/component/option,com_api/format,json/method,/")
            TRUE
        },
        error = function(e){
            FALSE
